...
Then, we will compare our Logistic Regression and Random Forest, particularly their performance when running on the test data set that we separated, and obtain one final candidate to be our ultimate winner. For the purposes of simplicity, we are splitting our original dataset 80-20 (as a rule of thumb) to create our training and testing sets. 
...
```{r, echo=FALSE}
#Balance
ggplot(bank.data,aes(x=balance,fill=deposit.status)) + geom_histogram(binwidth=1) +
  labs(y= "Number of Clients", x="Balance", title = "Distribution of Deposits by Balance")
...
To answer our question of interest, building a predictive categorical model, we choose to utilize two different methods. 
...
Reasons for selecting these 2 predictive techniques:
...
## First Method: Correlation & Heat Map & Explore Function

## Second Method: Stepwise (Forward, Backward, Both) Variable Selection using AIC, BIC, and Likelihood Ratio Test (LRT) for our Logistic Regression Model. Specifically, we have computed results for: 
...
## Third Method: SVM Method (See Below)

# 4. Results

## Method 1. Logistic Regression & Model Selection

Here is a summary of the logistic regression model with all 16 of the input variables (more interpretation of the output below). However, we would like to start with variable selection first. 
```{r, echo=FALSE}
#make copy of data
...
```
According to this criteria, we excluded previous, default, age, pdays, and job. Our model is y ~ duration + poutcome + month + contact + housing + campaign + loan + marital + education + balance + day.
...
According to this criteria, we excluded previous, default, age, pdays, job. Our model is y ~ marital + education + balance + housing + loan + contact + day + month + duration + campaign + poutcome. Which is the same as Forward Selection using BIC. 
...
#previous, default, age, pdays, job
```
Here, we exclude previous, default, age, pdays, and job. Our model is y ~ duration + poutcome + month + contact + housing + campaign + loan + marital + education + balance + day.
...
The confusion matrix is, and the corresponding classification accuracy is `r log.acc`
...
AUC - ROC curve is also a performance measurement for classification models. ROC is a probability curve and AUC represents degree or measure of separability. It tells how much model is capable of distinguishing between classes. Higher the AUC, the better the model accuracy. Therefore, our model achieved also 90% of accuracy as well, signifying that we have successfully pruned down the variables that are important.
...
```
From the importance plots, we see that duration is the most important according to both "MeanDecreaseAccuracy" and "MeanDecreaseGini" whereas default are the worst in both cases. This somewhat corresponds with our results obtained from the stepwise variable seleciton process. 

Furthermore, the corresponding confusion matrix is and the corresponding classification accuracy is `r classification_accuracy_rf`, very close to what we get in our logistic model after AIC variable selection. 
```{r}
conf_rf
```

However, the AUC that we computed from the ROC plot had a rather significant increase to `r AUC`. This is almost 3% higher than what we have obtained from our logistic model.
...
auc.svm=auc.svm@y.values[[1]];auc.svm
plot(svm.perf)
plot(svm.perf.acc)
conf.svm = table(pred.svm, as.factor(test.svm$y))
acc.svm = (conf.svm[1,1]+conf.svm[2,2])/(sum(conf.svm));acc.svm
```
...
For the SVM model, we decided to use a relatively smaller subset of the actual dataset. This was because it is computationally costly to run SVM and since this was a very huge dataset, it made more sense to use the subset of the dataset which we named "bank_small".

The accuracy that we obtained from running this model on the "bank_small" is `r acc.svm`. We can see that this is less than the accuracy of the logistic regression model that we trained which was `r log.acc`. Additionally, we found that it was also less than the classification accuracy of the random forest model which is `r classification_accuracy_rf`.

Moreover, from doing additional computation, we found out the Area Under the Curve for the ROC curve for each of the models. This area told us how much each of the models is capable of distinguishing between each of the classes. The AUC we got for the SVM model is `r auc.svm` as compared to the Logistic Regression AUC which was `r auc.log`.

Additionally, after doing some computation for precision score, we found out that the precision score for SVM model is .9016. We found this out by doing (True Positive)/(True Positive + False Positive). As compared to this, the precision score for Logistic Regression model is 0.917958. So we see that the precision score is higher for the Logistic Regression model as compared to the SVM. 
...
```{r, message=FALSE, warning = FALSE, echo = FALSE}
comparison.table = data.frame(Method = c('Random Forest', 'Logistic Regression'), Accuracy = NA, AUC = NA )
comparison.table$Accuracy = c(classicitation_accuracy_rf, log.acc)
comparison.table$AUC = c(AUC, auc.log)
