...
bank_small <- read_delim("datasets/bank.csv", 
...
```{r, echo=FALSE}
#Balance
ggplot(bank.data,aes(x=bank.data$balance,fill=deposit.status)) + geom_histogram(binwidth=1) +
...
We chose logistic regression as one of the modeling technique we would perform due to a couple of factors.It is relatively easy to implement on categorical data and the parameters can explain  the significance of each predictor variable. However, we still do acknowledge some of the down sides to this modeling technique. Linearity of the variables can affect the accuracy of the model and proper selection of features is required.

Random forest: 

The reason we chose random forest for this predictive model was because random forest is much better than logistic regression at handling multi-colinearity. The random forest model can also deal with categorical data better than the logistic regression can. However, THe logistic regression model does a better job deriving the significance of each of the predictor variables which would give us better incites on the data.
...
##SVM
```{r}
split = sample.split(bank_small$y,SplitRatio = 0.70)
...
The accuracy that we obtained from running this model on the "bank_small" is `r acc.svm`. We can see that this is less than the accuracy of the logistic regression model that we trained which was `r acc.log`. Additionally, we found that it was also less than the classification accuracy of the random forest model which is `r classification_accuracy`.
