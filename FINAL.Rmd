...
pred.tune.rf = prediction(as.numeric(pred3), bank.test$y)
...
acc.rf = performance(pred.tune.rf, measure = "acc")@y.values
acc.rf #accuracy
plot(accuracy.rf)
plot(perf.tune.rf) 
...
predictions <- predict(rf, newdata = bank.test, type = "prob")
pred.tune.rf = prediction(as.numeric(predictions), bank.test$y)
perf.tune.rf = performance(pred.tune.rf, measure = "tpr", x.measure = "fpr")
accuracy.rf = performance(pred.tune.rf, measure = "acc", x.measure = "cutoff")
acc.rf = performance(pred.tune.rf, measure = "acc")@y.values
acc.rf #accuracy
