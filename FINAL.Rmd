...
```{r, message=FALSE,warning=FALSE}
#sample 50% of data as train
train = sample(1:nrow(bank.data), 22605)

#create random forest model with all variables
rf <- randomForest(as.factor(y)~ ., data = bank.data, subset = train, importance = TRUE)
...
for(mtry in 1:16){
  fit = randomForest(as.factor(y) ~., data = bank.data, subset = train, mtry=mtry)
  oob.err[mtry] = fit$mse[1]
  pred = predict(fit, bank.data[-train,])
  test.err[mtry] = with(bank.data[-train,], mean( (y-pred)^2 ))
}
